...
GLFWwindow* Editor::Platform::window = nullptr;
int Editor::Platform::width = 0;
int Editor::Platform::height = 0;

int Editor::Platform::init(int argc, char **argv){
    // Initialize GLFW
    if (!glfwInit())
        return -1;
    ...
    window = glfwCreateWindow(1280, 720, "Supernova Editor", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
...
void Editor::Platform::disableMouseCursor(){
    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
}

void Editor::Platform::enableMouseCursor(){
