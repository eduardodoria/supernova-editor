...
std::map<std::string, Editor::PropertyData> Editor::Catalog::getProperties(ComponentType component, void* compRef){
    ...
    if(component == ComponentType::Transform){
        ...
        for (int s = 0; s < ((compRef) ? comp->numSubmeshes : 1); s++){
            ...
            ps["submeshes["+idx+"].material.basecolortexture"] = {PropertyType::Texture, "Base Texture", UpdateFlags_None, (void*)&def->submeshes[0].material.baseColorTexture, (compRef) ? (void*)&comp->submeshes[s].material.baseColorTexture : nullptr};
            ps["submeshes["+idx+"].material.emissivetexture"] = {PropertyType::Texture, "Emissive Texture", UpdateFlags_None, (void*)&def->submeshes[0].material.emissiveTexture, (compRef) ? (void*)&comp->submeshes[s].material.emissiveTexture : nullptr};
            ps["submeshes["+idx+"].material.metallicroughnesstexture"] = {PropertyType::Texture, "Met. Rou. Texture", UpdateFlags_None, (void*)&def->submeshes[0].material.metallicRoughnessTexture, (compRef) ? (void*)&comp->submeshes[s].material.metallicRoughnessTexture : nullptr};
            ps["submeshes["+idx+"].material.occlusiontexture"] = {PropertyType::Texture, "Occlusion Texture", UpdateFlags_None, (void*)&def->submeshes[0].material.occlusionTexture, (compRef) ? (void*)&comp->submeshes[s].material.occlusionTexture : nullptr};
            ps["submeshes["+idx+"].material.normalTexture"] = {PropertyType::Texture, "Normal Texture", UpdateFlags_None, (void*)&def->submeshes[0].material.normalTexture, (compRef) ? (void*)&comp->submeshes[s].material.normalTexture : nullptr};
